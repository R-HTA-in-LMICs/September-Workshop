...
This GitHub repository provides the code of R-HTA in LMICs September tutorial on how to implement time-dependent cohort state-transition models (cSTMs) in R using a cost-effectiveness analysis (CEA) example. The tutorial is based on the open-source DARTH group materials, and this specific tutorial is explained in the following manuscript: 
...
The [`analysis`](https://github.com/DARTH-git/cohort-modeling-tutorial-timedep/tree/main/analysis) folder includes the scripts with all the code, description and comments to reproduce the CEA, probabilistic sensitivity analysis (PSA) and generation of epidemiological measures of the manuscript:
   ...
   - [`cSTM_torn_diag.R`](https://github.com/DARTH-git/cohort-modeling-tutorial-timedep/blob/main/R/cSTM_torn_diag.R): Code to implement a tornado diagnram analysis for DSA.
...
We recommend reading the DARTH group's introductory tutorial on time-independent cSTMs in R:
...
and, although more technical, try to understand the use of multidimensional arrays to represent cSTM dynamics in R described in:
...
lastly, familiarizing with the [DARTH](http://darthworkgroup.com) coding framework described in:
